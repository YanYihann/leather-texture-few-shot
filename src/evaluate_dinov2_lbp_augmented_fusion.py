...
class AugmentedFusionDataset(Dataset):
    def __init__(self, dataset_dir, views_per_class, base_transform, tensor_transform):
        self.base = BaseLeatherDataset(dataset_dir)
    ...
    def __getitem__(self, index):
        ...
        augmented_image = self.base_transform(image)
        model_image = self.tensor_transform(augmented_image)
        lbp_hist = extract_lbp_histogram(augmented_image)
        return model_image, lbp_hist, label, class_name, str(image_path), view_index


class EvalFusionDataset(Dataset):
    def __init__(self, dataset_dir, base_transform, tensor_transform):
        self.base = BaseLeatherDataset(dataset_dir)
        self.class_names = self.base.class_names
        self.base_transform = base_transform
        self.tensor_transform = tensor_transform
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(image_path).convert("RGB")
        eval_image = self.base_transform(image)
        model_image = self.tensor_transform(eval_image)
        lbp_hist = extract_lbp_histogram(eval_image)
...
def build_augmented_image_transform(image_size):
...
def build_eval_image_transform(image_size):
    return transforms.Compose(
        [
...
def build_augmented_prototypes(model, dataset, batch_size, device, lbp_weight):
    ...
    with torch.no_grad():
        for images, lbp_features, labels, class_names, image_paths, view_indices in loader:
            ...
            if feature_sum is None:
                feature_sum = torch.zeros(
                    (len(dataset.class_names), fused.shape[1]),
                    dtype=torch.float32,
                )

            for feature, label, image_path in zip(fused, labels.tolist(), image_paths):
...
def extract_eval_features(model, dataset, batch_size, device, lbp_weight):
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
    all_features = []
    all_labels = []
    all_class_names = []
...
def evaluate(args):
    ...
    train_dataset = AugmentedFusionDataset(
        args.train_dir,
        args.views_per_class,
        build_augmented_image_transform(args.image_size),
        tensor_transform,
    )
    val_dataset = EvalFusionDataset(
        args.val_dir,
        build_eval_image_transform(args.image_size),
        tensor_transform,
    )
    ...
    feature_path = Path(
        args.feature_path
        or f"features/{args.model_name}_lbp_augmented_N{args.views_per_class}_w{safe_weight}.pt"
    )
    feature_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "method": f"{args.model_name} + LBP augmented prototype fusion",
    ...
    val_data = extract_eval_features(
        model,
        val_dataset,
        args.batch_size,
        device,
    ...
    metrics = {
        ...
        "train_source_images": len(train_dataset.base.samples),
    ...
    metrics_path = (
        output_dir
        / f"stage5_{args.model_name}_lbp_augmented_N{args.views_per_class}_w{safe_weight}_metrics.json"
    )
    predictions_path = (
        output_dir
        / f"stage5_{args.model_name}_lbp_augmented_N{args.views_per_class}_w{safe_weight}_predictions.csv"
    )
...
def parse_args():
    ...
    parser.add_argument("--lbp-weight", type=float, default=0.1)
