...
def build_image_prototypes(ref_data, num_classes):
    dim = ref_data["features"].shape[1]
    sums = torch.zeros((num_classes, dim), dtype=torch.float32)
    counts = torch.zeros(num_classes, dtype=torch.float32)
    for feature, label in zip(ref_data["features"], ref_data["labels"]):
        sums[label] += feature
        counts[label] += 1
    return F.normalize(sums / counts.unsqueeze(1), p=2, dim=1)


def encode_text_prototypes(model, tokenizer, class_names, device):
...
def write_result(output_dir, split, method, eval_dir, result):
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_method = method.lower().replace(" ", "_").replace("+", "plus").replace("/", "_")
    metrics = {
        "split": split,
        "method": method,
    ...
    (output_dir / f"{split}_{safe_method}_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    with (output_dir / f"{split}_{safe_method}_predictions.csv").open("w", newline="", encoding="utf-8-sig") as file:
...
def main():
    ...
    parser.add_argument("--output-dir", default="results/generated/open_clip_baselines")
    ...
    model.eval()
    reference = LeatherDataset(args.reference_dir, transform=preprocess)
    ...
    text_prototypes, prompts = encode_text_prototypes(model, tokenizer, reference.class_names, device)

    split_dirs = {"spatial": args.spatial_dir, "independent": args.independent_dir}
    summary = []
    start = time.perf_counter()
    for split in args.splits:
        evaluation = LeatherDataset(split_dirs[split], class_names=reference.class_names, transform=preprocess)
        eval_data = encode_images(model, evaluation, args.batch_size, device)
        for method, prototypes in [
            ...
            metrics = write_result(Path(args.output_dir), split, method, split_dirs[split], result)
            metrics["elapsed_seconds_total"] = round(time.perf_counter() - start, 3)
            summary.append(metrics)
            print(json.dumps(metrics, indent=2))

    with (Path(args.output_dir) / "clip_baselines_summary.csv").open("w", newline="", encoding="utf-8-sig") as file:
    ...
    (Path(args.output_dir) / "clip_zero_shot_prompts.json").write_text(json.dumps(prompts, indent=2), encoding="utf-8")
